...
%  Output        : quant (Quantized Coefficients, 8x8x3)
    % L is table for luminance components
    % C is table for chrominance components
    ...
    quant(:,:,1) = round(dct_block(:,:,1)./(L*qScale)); % luminance components
    quant(:,:,2) = round(dct_block(:,:,2)./(C*qScale)); % chrominance components
    quant(:,:,3) = round(dct_block(:,:,3)./(C*qScale)); % chrominance components
end