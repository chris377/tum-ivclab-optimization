...
%  Output        : dct_block    (Dequantized DCT coefficients, 8x8x3)
    ...
    dct_block(:,:,1) = quant_block(:,:,1).*(L*qScale); % luminance components
    dct_block(:,:,2) = quant_block(:,:,2).*(C*qScale); % chrominance components
    dct_block(:,:,3) = quant_block(:,:,3).*(C*qScale); % chrominance components    
